...
export const buildAndroidGradleTask = (props: BugSplatPluginOptions): string => {
  ...
  return [
    '',
    '// BugSplat symbol upload task. Inside doLast, "this" is the Task — so',
    '// Project methods (exec, fileTree, layout, android extension) must be',
    '// captured at configuration time or qualified with project.* to survive',
    '// newer Gradle versions that no longer delegate from Task to Project.',
    ...
    '    def execAction = project.&exec',
    '    def fileTreeFactory = project.&fileTree',
    '',
    '    doLast {',
    ...
    '        def hasMaps = sourceMapDir.exists() && fileTreeFactory(sourceMapDir).matching { include "**/*.js.map", "**/*.bundle.map" }.files.size() > 0',
    '        if (hasMaps) {',
    '            execAction({',
    '                commandLine("npx", "--yes", "@bugsplat/symbol-upload",',
    '                    "-b", bsDatabase,',
