...
export const buildAndroidGradleTask = (props: BugSplatPluginOptions): string => {
  ...
  return [
    ...
    'tasks.register("uploadBugsplatSymbols", Exec::class) {',
    `    val bsDatabase = ${database}`,
    `    val bsClientId = ${clientId}`,
    `    val bsClientSecret = ${clientSecret}`,
    '    val appName = android.defaultConfig.applicationId ?: project.name',
    '    val appVersion = android.defaultConfig.versionName ?: "1.0.0"',
    '    val soDir = layout.buildDirectory.dir("intermediates/merged_native_libs").get().asFile.absolutePath',
    '',
